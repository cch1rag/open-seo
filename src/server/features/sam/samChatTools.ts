...
function adaptMcpTool<Shape extends ZodRawShape>(
  ...
  return tool({
    ...
    execute: async (args) => {
      ...
      try {
        // A rewind can cancel the turn between Think dispatching this call and
        // its execute() actually starting. beforeToolCall already blocks any
        // NEW call once Think's own signal is aborted; this catches the case
        // where this one had already started dispatching. No handler reads a
        // signal to cut short in-flight work (none accepts one), so this is a
        // pre-flight check, not a mid-call abort.
        if (getAbortSignal?.()?.aborted) {
          throw new Error("SAM was cancelled");
        }
...
export function buildSamMcpTools(
  ...
  ) => adaptMcpTool(definition, toolContext, projectId, getAbortSignal);
