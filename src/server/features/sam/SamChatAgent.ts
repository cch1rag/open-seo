...
} from "@/server/features/sam/samContextBlocks";
import {
  deriveTitle,
  firstUserText,
...
export class SamChatAgent extends Think {
  ...
  // Routed through the shared provider selection so a self-hosted
  // OpenAI-compatible endpoint works here too, not just OpenRouter.
  private buildModel(reasoningEffort: "max" | "low") {
    return getChatAgentModelSync(this.env, reasoningEffort);
  }
  ...
  configureSession(session: Session): Session {
    return session
      .withContext("soul", {
        provider: { get: () => buildSoulPrompt(() => this.loadSamContext()) },
      })
      .withContext(PROJECT_CONTEXT_BLOCK, {
        ...
        provider: {
          get: () => renderProjectContext(() => this.loadSamContext()),
        },
      })
      .onCompaction(
  ...
  async beforeTurn(turnCtx: TurnContext): Promise<TurnConfig> {
    ...
    return withPgClient(async (): Promise<TurnConfig> => {
      ...
      // Step and output budgets sit below upstream's (40 / 16k) on purpose:
      // the tool policy below already bounds a turn's paid work, and the
      // tighter caps keep a single turn well clear of the Durable Object
      // memory limit. Both stay above measured reasoning use (~3k tokens),
      // which a tight cap would let eat the visible reply (issue #161).
      return {
        tools: samTools,
        activeTools: getSamActiveToolNames({
          ...turnCtx.tools,
          ...samTools,
        }),
  ...
  beforeToolCall(ctx: ToolCallContext) {
    ...
    return block
      ? { action: "block" as const, reason: block.reason }
      : undefined;
  }
